...
int gr_area_light_cmd(lua_State* L)
{
    ...
    get_tuple(L, 2, col, 3);
    get_tuple(L, 3, al.falloff, 3);
     al.radius = luaL_checknumber(L, 4);

    
    al.colour = glm::vec3(col[0], col[1], col[2]);
    
    data->light = new AreaLight(al.colour, al.position, al.radius);
...
int gr_node_set_texture_cmd(lua_State* L)
{
    ...
    std::string texture_file_name = luaL_optstring(L, 2, "Assets/sample.png");

    RasterImage *texture = new RasterImage(texture_file_name);
