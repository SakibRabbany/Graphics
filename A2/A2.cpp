...
void A2::guiLogic()
{
    ...
    ImGui::Text( "Near: %.1f FPS", near );
    ImGui::Text( "Far: %.1f FPS", far );
    ImGui::End();
}
...
bool A2::mouseMoveEvent (
    ...
    if (!ImGui::IsMouseHoveringAnyWindow()) {
        ...
        if (viewport_mode & left_mouse_pressed) {
            dragged = true;
            if (dragged) {
                view_port_end = m_mouse_GL_coordinate;
                updateViewport();
            }
...
bool A2::mouseButtonInputEvent (
    ...
    if (actions == GLFW_PRESS) {
        ...
        if (button == GLFW_MOUSE_BUTTON_LEFT) {
            ...
            if (user_mode == 6) {
                viewport_mode = true;
                view_port_start = m_mouse_GL_coordinate;
            }
...
glm::mat4 A2::rotate(const glm::mat4 &mat, float angle, A2::Axis axis)
{
    ...
    return   mat * rotate_matrix ;
    
}

glm::mat4 A2::look_at(const glm::vec3 &camera_position, const glm::vec3 &center, const glm::vec3 &up)
{
    vec3 z_direction = (center - camera_position)/length(center - camera_position);
//    vec3 u = normalize(up);
    vec3 x_direction = (cross(up, z_direction))/length((cross(up, z_direction)));
    
    vec3 y = cross(z_direction, x_direction);
...
void A2::drawCube() {
    ...
    for (const pair<vec4, vec4>& line : m_cube_lines) {
        ...
        if (clip(p1, p2)){
            ...
            mapWorldToViewPortAndDraw(vec2(v1.x, v1.y), vec2(v2.x, v2.y));
        }
...
void A2::drawModelGnomon() {
    ...
    for (const pair<vec4, vec4>& line : m_cube_gnomon_lines) {
        if (i == 0) setLineColour(vec3(0.6,0,0));
        if (i == 1) setLineColour(vec3(0,0.6,0));
        if (i == 2) setLineColour(vec3(0,0,0.6));
...
void A2::drawWorldGnomon(){
    ...
    for (const pair<vec4, vec4>& line : m_world_gnomon_lines) {
        ...
        if (i == 1) setLineColour(vec3(0,1,0));
        if (i == 2) setLineColour(vec3(0,0,1));
...
void A2::drawViewport(){
    ...
    for (const pair<vec2, vec2>& line : m_viewport) {
...
void A2::updateMatrices(vec2& delta, Axis axis) {
    ...
    if (user_mode == 3) {
        V =  (rotate(mat4(1), delta.x, axis)) * V ;
    }
    ...
    if (user_mode == 5) {
        if (axis == Axis::x_axis) {
            field_of_view += delta.x;
            if (field_of_view < 5) field_of_view = 5;
            if (field_of_view > 160) field_of_view = 160;
        }
        if (axis == Axis::y_axis) {
            near += delta.x;
        }
        ...
        P = perspective(field_of_view, m_framebufferWidth/m_framebufferHeight, near, far);
...
void A2::makeViewport() {
    m_viewport.clear();
    
    vec2 top_left (view_port_center.x - view_port_width, view_port_center.y + view_port_height);
    vec2 top_right(view_port_center.x + view_port_width, view_port_center.y + view_port_height);
    vec2 bottom_left (view_port_center.x - view_port_width, view_port_center.y - view_port_height);
    vec2 bottom_right(view_port_center.x + view_port_width, view_port_center.y - view_port_height);

    m_viewport.push_back(make_pair(top_right, top_left));
    m_viewport.push_back(make_pair(top_left, bottom_left));
    m_viewport.push_back(make_pair(bottom_left, bottom_right));
    m_viewport.push_back(make_pair(bottom_right, top_right));
...
void A2::reset() {
    ...
    left_mouse_pressed = middle_mouse_pressed = right_mouse_pressed = viewport_mode = dragged = false;
    ...
    view_port_width = 0.95;
    view_port_height = 0.95;
    view_port_center = glm::vec4(0,0,0,1);
...
void A2::mapWorldToViewPortAndDraw(const glm::vec2 &p1, const glm::vec2 &p2)
{
    mat4 scale_mat = scale(mat4(1), vec3(view_port_width, view_port_height, 1));
    mat4 translation_mat = translate(mat4(1), vec3(view_port_center.x, view_port_center.y, 0));
    
    vec4 point1(p1.x, p1.y, 0, 1);
    vec4 point2(p2.x, p2.y, 0, 1);
    
    point1 = translation_mat * scale_mat * point1;
    point2 = translation_mat * scale_mat * point2;
    
    drawLine({point1.x, point1.y}, {point2.x, point2.y});
}

void A2::updateViewport(){
    view_port_end.x = view_port_end.x > 1 ? 1 : view_port_end.x;
    view_port_start.x = view_port_start.x < -1 ? -1 : view_port_start.x;

    view_port_end.y = view_port_end.y > 1 ? 1 : view_port_end.y;
    view_port_start.y = view_port_start.y < -1 ? -1 : view_port_start.y;

    
    view_port_height = abs(view_port_end.y - view_port_start.y)/2;
    view_port_width = abs(view_port_end.x - view_port_start.x)/2;
    view_port_center = vec4((view_port_end.x + view_port_start.x)/2, (view_port_end.y + view_port_start.y)/2,0,1);
    makeViewport();
}
